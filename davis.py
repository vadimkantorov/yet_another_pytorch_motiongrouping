...
class DAVIS(torchvision.datasets.VisionDataset):
    def __init__(self, root, split_name, year = 2017, resolution = '480p', root_flow = None, transform = torchvision.transforms.ToTensor(), dt = [0, -2, -1, 1, 2], fmt = '{:05d}.jpg', filter = None, pad = lambda tensor, multiple = 8: tensor[..., : (tensor.shape[-2] // multiple) * multiple, : (tensor.shape[-1] // multiple) * multiple] ):
