import os
import argparse
import functools
import multiprocessing
import numpy as np
import cv2
import torch
import torchvision
from PIL import Image

class DAVIS(torchvision.datasets.VisionDataset):
    def __init__(self, root, split_name, year = 2016, resolution = '480p', root_flow = None, transform = torchvision.transforms.ToTensor(), dt = [0, -2, -1, 1, 2], fmt = '{:05d}.jpg', read_frames = True, pad = lambda tensor, multiple = 8: tensor[..., : (tensor.shape[-2] // multiple) * multiple, : (tensor.shape[-1] // multiple) * multiple] ):
        super().__init__(root, transform = transform)
        self.resolution = resolution
        self.pad = pad
        self.read_frames = read_frames

        self.classes = list(map(str.strip, open(os.path.join(root, 'ImageSets', str(year), split_name + '.txt'))))
        frame_count = { class_name : 1 + int(os.path.splitext(sorted(os.listdir(os.path.join(root, 'JPEGImages', resolution, class_name)))[-1])[0] ) for class_name in self.classes }

        frame_path = lambda class_name, t: os.path.join(root, 'JPEGImages', resolution, class_name, fmt.format(t))
        flow_path = lambda class_name, t, t_n: os.path.join(root_flow, 'JPEGImages', resolution, class_name, (fmt + '_' + fmt).format(t, t_n))

        self.frames = [ list(frame_path(class_name, min(max(0, t + n), cnt - 1)) for n in dt) for class_name, cnt in frame_count.items() for t in range(cnt) ]
        self.frames_flow = [ list(flow_path(class_name, t, min(max(0, t + n), cnt - 1)) for n in dt) for class_name, cnt in frame_count.items() for t in range(cnt) ] if root_flow else ([()] * len(self.frames))

        # metadata DAVIS/Annotations/480p/lucia/00000.png

    def __getitem__(self, idx):
        # TODO: fill out non-existing first element
        def read_stack(paths, **kwargs):
            if paths and all(map(os.path.exists, paths[1:])):
                imgs = [ torch.as_tensor(cv2.imread(frame_path)) for frame_path in paths[1:] ]
                imgs.insert(0, torch.as_tensor(cv2.imread(paths[0])) if os.path.exists(paths[0]) else torch.zeros_like(imgs[0]))
                return self.pad(torch.stack(imgs).movedim(-1, 1)).flip(1).div(255.0) 
            else:
                return torch.empty(**kwargs)
        
        if self.read_frames:
            frames = read_stack(self.frames[idx])
            frames_flow = read_stack(self.frames_flow[idx], size = frames.shape, dtype = frames.dtype)
        else:
            frames_flow = read_stack(self.frames_flow[idx])
            frames = read_stack(self.frames[idx], size = frames_flow.shape, dtype = frames_flow.dtype)

        return self.frames[idx], frames, self.frames_flow[idx], frames_flow

    def __len__(self):
        return len(self.frames)

palette_str = '''0 0 0
128 0 0
0 128 0
128 128 0
0 0 128
128 0 128
0 128 128
128 128 128
64 0 0
191 0 0
64 128 0
191 128 0
64 0 128
191 0 128
64 128 128
191 128 128
0 64 0
128 64 0
0 191 0
128 191 0
0 64 128
128 64 128
22 22 22
23 23 23
24 24 24
25 25 25
26 26 26
27 27 27
28 28 28
29 29 29
30 30 30
31 31 31
32 32 32
33 33 33
34 34 34
35 35 35
36 36 36
37 37 37
38 38 38
39 39 39
40 40 40
41 41 41
42 42 42
43 43 43
44 44 44
45 45 45
46 46 46
47 47 47
48 48 48
49 49 49
50 50 50
51 51 51
52 52 52
53 53 53
54 54 54
55 55 55
56 56 56
57 57 57
58 58 58
59 59 59
60 60 60
61 61 61
62 62 62
63 63 63
64 64 64
65 65 65
66 66 66
67 67 67
68 68 68
69 69 69
70 70 70
71 71 71
72 72 72
73 73 73
74 74 74
75 75 75
76 76 76
77 77 77
78 78 78
79 79 79
80 80 80
81 81 81
82 82 82
83 83 83
84 84 84
85 85 85
86 86 86
87 87 87
88 88 88
89 89 89
90 90 90
91 91 91
92 92 92
93 93 93
94 94 94
95 95 95
96 96 96
97 97 97
98 98 98
99 99 99
100 100 100
101 101 101
102 102 102
103 103 103
104 104 104
105 105 105
106 106 106
107 107 107
108 108 108
109 109 109
110 110 110
111 111 111
112 112 112
113 113 113
114 114 114
115 115 115
116 116 116
117 117 117
118 118 118
119 119 119
120 120 120
121 121 121
122 122 122
123 123 123
124 124 124
125 125 125
126 126 126
127 127 127
128 128 128
129 129 129
130 130 130
131 131 131
132 132 132
133 133 133
134 134 134
135 135 135
136 136 136
137 137 137
138 138 138
139 139 139
140 140 140
141 141 141
142 142 142
143 143 143
144 144 144
145 145 145
146 146 146
147 147 147
148 148 148
149 149 149
150 150 150
151 151 151
152 152 152
153 153 153
154 154 154
155 155 155
156 156 156
157 157 157
158 158 158
159 159 159
160 160 160
161 161 161
162 162 162
163 163 163
164 164 164
165 165 165
166 166 166
167 167 167
168 168 168
169 169 169
170 170 170
171 171 171
172 172 172
173 173 173
174 174 174
175 175 175
176 176 176
177 177 177
178 178 178
179 179 179
180 180 180
181 181 181
182 182 182
183 183 183
184 184 184
185 185 185
186 186 186
187 187 187
188 188 188
189 189 189
190 190 190
191 191 191
192 192 192
193 193 193
194 194 194
195 195 195
196 196 196
197 197 197
198 198 198
199 199 199
200 200 200
201 201 201
202 202 202
203 203 203
204 204 204
205 205 205
206 206 206
207 207 207
208 208 208
209 209 209
210 210 210
211 211 211
212 212 212
213 213 213
214 214 214
215 215 215
216 216 216
217 217 217
218 218 218
219 219 219
220 220 220
221 221 221
222 222 222
223 223 223
224 224 224
225 225 225
226 226 226
227 227 227
228 228 228
229 229 229
230 230 230
231 231 231
232 232 232
233 233 233
234 234 234
235 235 235
236 236 236
237 237 237
238 238 238
239 239 239
240 240 240
241 241 241
242 242 242
243 243 243
244 244 244
245 245 245
246 246 246
247 247 247
248 248 248
249 249 249
250 250 250
251 251 251
252 252 252
253 253 253
254 254 254
255 255 255'''

def color2id(c, palette8 = np.array([[int(x) for x in line.split()] for line in palette_str.split('\n')], dtype = np.uint8)):
    return np.arange(0, palette8.shape[0])[np.all(palette8 == c, axis=-1)]

def process_video(basename, args):
    gt_dir = os.path.join(args.root, 'Annotations', args.resolution, basename)
    output_dir = os.path.join(args.output_dir, basename)

    os.mkdir(output_dir, exist_ok = True)

    img_files = [basename for basename in os.listdir(gt_dir) if basename.endswith('.png')]

    lblimg  = cv2.imread(os.path.join(gt_dir, args.fmt.format(0)))
    height, width = lblimg.shape[:2]

    for j in range(len(img_files)):
        output_path = os.path.join(output_dir, args.fmt.format(j))
        input_path = os.path.join(args.input_dir, f'{i}_{j}_mask.png')

        lblimg = cv2.imread(input_path)
        lblidx = np.zeros(lblimg.shape[:2])

        for color in np.unique(lblimg.reshape(-1, 3), axis = 0):
            cid = color2id(color)
            if len(cid) > 0:
                lblidx[np.all(lblimg == color, axis = -1)] = cid

        lblidx = cv2.resize(lblidx.astype(np.uint8), (width, height), interpolation = cv2.INTER_NEAREST).astype(np.uint8)

        img = Image.fromarray(lblidx)
        img.putpalette(palette.ravel())
        img.save(output_path, format = 'PNG')


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('-o', '--output-dir', type=str)
    parser.add_argument('-i', '--input-dir', type=str)
    parser.add_argument('--root', type=str)
    parser.add_arguments('-j', 'num_workers', default = 10, type = int)
    parser.add_argument('--fmt', default = '{:05d}.png') 
    parser.add_argument('--resolution', default = '480p')
    parser.add_argument('--year', type = int, default = 2017)
    parser.add_argument('--split', default = 'val.txt', choices = ['train.txt', 'val.txt', 'test.txt'])
    args = parser.parse_args()

    os.makedirs(args.output_dir, exist_ok = True)
    
    imglist = list(map(str.strip, open(os.path.join(args.root, 'ImageSets', args.year, args.split), 'r')))
    multiprocessing.Pool(args.num_workers).map(functools.partial(process_video, args = args), imglist)
